 ...
 *
 * Redesigned eye (layered iris fibers / crypts / collarette / limbal ring,
 * wet catchlights, spherical sclera, corner-biased veins, realistic tears)
 * and a darker crimson apocalypse scene (LCL fog, god-rays, embers, light
 * cracks, heavy bloom). Tuning values are the locked "Weeping Void" preset.
 */
@RequiresApi(Build.VERSION_CODES.TIRAMISU)
internal class WeepingVoidShader private constructor() {
    ...
    fun DrawScope.drawWeepingVoid(light: Boolean, time: Float) {
        synchronized(shader) {
            ...
            shader.setFloatUniform("u_quality", 0f) // ECO preset (3-octave noise)
            drawRect(brush = brush)
        }
    }

    companion object {
        private var instance: WeepingVoidShader? = null

        fun getInstance(): WeepingVoidShader {
            return instance ?: synchronized(this) {
                instance ?: WeepingVoidShader().also { instance = it }
            }
        }
    }
}
