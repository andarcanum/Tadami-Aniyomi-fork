...
class MangaHistoryRepositoryImpl(
    ...
    override fun getMangaHistory(query: String): Flow<List<MangaHistoryWithRelations>> {
        return handler.subscribeToList { db ->
            db.historyViewQueries.history(query, MangaHistoryMapper::mapMangaHistoryWithRelations)
        }
    }

    override suspend fun getLastMangaHistory(): MangaHistoryWithRelations? {
        return handler.awaitOneOrNull { db ->
            db.historyViewQueries.getLatestHistory(MangaHistoryMapper::mapMangaHistoryWithRelations)
        }
    }

    override suspend fun getTotalReadDuration(): Long {
        return handler.awaitOne { db -> db.historyQueries.getReadDuration() }
    }
    ...
    override suspend fun resetMangaHistory(historyId: Long) {
        try {
            handler.await { db -> db.historyQueries.resetHistoryById(historyId) }
        } catch (e: Exception) {
    ...
    override suspend fun resetHistoryByMangaId(mangaId: Long) {
        try {
            handler.await { db -> db.historyQueries.resetHistoryByMangaId(mangaId) }
        } catch (e: Exception) {
            logcat(LogPriority.ERROR, throwable = e)
        }
    }
    ...
    override suspend fun deleteAllMangaHistory(): Boolean {
        return try {
            handler.await { db -> db.historyQueries.removeAllHistory() }
            true
        } catch (e: Exception) {
            logcat(LogPriority.ERROR, throwable = e)
            false
