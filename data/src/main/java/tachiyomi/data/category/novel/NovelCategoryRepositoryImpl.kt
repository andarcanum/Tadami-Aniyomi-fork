...
class NovelCategoryRepositoryImpl(
    ...
    override suspend fun getCategory(id: Long): NovelCategory? {
        ...
        return handler.awaitOneOrNull { db -> db.novel_categoriesQueries.getCategory(id, ::mapCategory) }
    }

    override suspend fun getCategories(): List<NovelCategory> {
        migrateLegacyCategoriesIfNeeded()
        return handler.awaitList { db -> db.novel_categoriesQueries.getCategories(::mapCategory) }
    }

    override suspend fun getVisibleCategories(): List<NovelCategory> {
        migrateLegacyCategoriesIfNeeded()
        return handler.awaitList { db -> db.novel_categoriesQueries.getVisibleCategories(::mapCategory) }
    }

    override suspend fun getCategoriesByNovelId(novelId: Long): List<NovelCategory> {
        migrateLegacyCategoriesIfNeeded()
        return handler.awaitList { db ->
            db.novel_categoriesQueries.getCategoriesByNovelId(novelId, ::mapCategory)
        }
    }
    ...
    override suspend fun getVisibleCategoriesByNovelId(novelId: Long): List<NovelCategory> {
        migrateLegacyCategoriesIfNeeded()
        return handler.awaitList { db ->
            db.novel_categoriesQueries.getVisibleCategoriesByNovelId(novelId, ::mapCategory)
        }
    }
    ...
    override fun getCategoriesAsFlow(): Flow<List<NovelCategory>> {
        return flow {
            migrateLegacyCategoriesIfNeeded()
            emitAll(handler.subscribeToList { db -> db.novel_categoriesQueries.getCategories(::mapCategory) })
        }
    }
    ...
    override fun getVisibleCategoriesAsFlow(): Flow<List<NovelCategory>> {
        return flow {
            migrateLegacyCategoriesIfNeeded()
            emitAll(handler.subscribeToList { db -> db.novel_categoriesQueries.getVisibleCategories(::mapCategory) })
        }
    }
    ...
    override suspend fun updateAllFlags(flags: Long) {
        migrateLegacyCategoriesIfNeeded()
        handler.await { db ->
            db.novel_categoriesQueries.updateAllFlags(flags)
        }
    }

    override suspend fun deleteCategory(categoryId: Long) {
        migrateLegacyCategoriesIfNeeded()
        handler.await { db -> db.novel_categoriesQueries.delete(categoryId) }
    }

    override suspend fun setNovelCategories(novelId: Long, categoryIds: List<Long>) {
        migrateLegacyCategoriesIfNeeded()
        handler.await(inTransaction = true) { db ->
            db.novels_categoriesQueries.deleteNovelCategoryByNovelId(novelId)
            categoryIds.map { categoryId ->
                db.novels_categoriesQueries.insert(novelId, categoryId)
            }
        }
    }
    ...
    private suspend fun migrateLegacyCategoriesIfNeeded() {
        ...
        handler.await(inTransaction = true) { db ->
            val legacyCategories = db.categoriesQueries.getCategories(::mapCategory)
                .executeAsList()
                .filterNot { it.id == 0L }
            legacyCategories.forEach { legacy ->
                db.novel_categoriesQueries.insertOrIgnoreWithId(
                    id = legacy.id,
                    name = legacy.name,
                    order = legacy.order,
                    flags = legacy.flags,
